...
- 현재 리브랜딩 2차 cleanup-lite 배포는 Madi 저장소와 iCloud 컨테이너를 기본값으로 쓰고, 기존 앱 데이터 보호를 위해 legacy 로컬 DB import와 legacy iCloud read-only import 경로만 유지합니다.
...
- 2차 cleanup-lite release: 현재 단계입니다. Madi는 legacy iCloud에 더 이상 쓰지 않고, late device 변경을 읽어 Madi 쪽으로 가져오는 read-only import 경로만 남깁니다. 업데이트하지 않은 기존 바이너리를 원격으로 강제 차단하지는 못합니다.
