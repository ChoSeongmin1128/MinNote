...
impl CloudKitBridge {
  ...
  pub fn get_account_status(&self) -> Result<ICloudAccountStatus, AppError> {
    let response = self.invoke::<serde_json::Value, AccountStatusResponse>("getAccountStatus", &serde_json::json!({}))?;
  ...
  fn invoke<TReq: Serialize, TResp: DeserializeOwned>(
    ...
    let mut child = Command::new(&self.executable_path)
      ...
      .stderr(Stdio::piped())
      .spawn()
      .map_err(|error| AppError::validation(format!("iCloud bridge를 실행하지 못했습니다: {error}")))?;
    ...
    let output = child
      .wait_with_output()
      .map_err(|error| AppError::validation(format!("iCloud bridge 응답을 기다리지 못했습니다: {error}")))?;
...
fn resolve_bridge_path() -> Result<PathBuf, AppError> {
  let target_triple = option_env!("MINNOTE_TARGET_TRIPLE")
    .map(str::to_string)
    .or_else(|| std::env::var("TARGET").ok())
    .unwrap_or_else(|| "aarch64-apple-darwin".to_string());
  let sidecar_name = format!("minnote-cloudkit-bridge-{target_triple}");
  let bundled_sidecar_name = "minnote-cloudkit-bridge";
  let bundled_helper_app = "minnote-cloudkit-bridge.app";
  let current_exe = std::env::current_exe()
    .map_err(|error| AppError::validation(format!("현재 실행 파일 경로를 확인하지 못했습니다: {error}")))?;

  let candidates = [
    current_exe
      .parent()
      .and_then(|path| path.parent())
      .map(|path| path.join("Resources").join(bundled_helper_app).join("Contents").join("MacOS").join(bundled_sidecar_name)),
    ...
    Some(PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("binaries").join(&sidecar_name)),
  ];
