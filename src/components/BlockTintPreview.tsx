...
interface BlockTintPreviewProps {
  ...
  className?: string;
  variant?: 'blocks' | 'swatches';
}

export function BlockTintPreview({
  preset,
  className,
  variant = 'blocks',
}: BlockTintPreviewProps) {
  ...
  return (
    ...
    >
      {variant === 'swatches' ? (
        <span className="document-menu-option-preview-swatches">
          <span className="document-menu-option-preview-swatch" />
          <span className="document-menu-option-preview-swatch is-alt" />
        </span>
      ) : (
